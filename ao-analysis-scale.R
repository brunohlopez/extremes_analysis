...
#Influence of the AO on the scale parameter
  ...
  for (d in 1:nlat) {
    ...
    if ( sum(is.na(x_lr))  == 0 && is.na(loc1[c,d]) == F && is.na(sca1[c,d]) == F && is.na(sha1[c,d]) == F) {
      if (sd(x_lr) >0){
        ...
        outlt_ao = fevd(x_lr,units = "deg C",scale.fun = ~ao) #looks at the scale parameter
        ao_effect[c,d] = outlt_ao$results$par[3] #third results is the ao influence
...
save(ao_effect, file = "ap_effect_scale")
save(lr_p_value_ao, file = 'lr_p_value_ao_scale.RData')

ind_ao = which(lr_p_value_ao < 0.05) #checks for 5 percent significance
ind_not_ao = which(lr_p_value_ao > 0.05)
#populates the matrix with the results
sig_or_not_ao[ind_ao] = 1 #If significant give a value of 1
  ...
  for (f in 1:nlat) {
    ...
    if ( sum(is.na(x_lr))  == 0 && is.na(loc1[e,f]) == F && is.na(sca1[e,f]) == F && is.na(sha1[e,f]) == F) {
      if (sd(x_lr) >0){
        ...
        out_lin_ao = fevd(x_lr, units = "deg C", scale.fun = ~ao + trend)
        outlt_ao = fevd(x_lr,units = "deg C", scale.fun = ~trend)
        lr_test_method = lr.test(out_lin_ao,outlt_ao)
        lr_p_value_ao_trend1[e,f] = lr_test_method$p.value
        the_ao_effect[e,f] = out_lin_ao$results$par[3] #the affect of the ao
        trend_effect[e,f] = out_lin_ao$results$par[4] #the affect of the location
...
save(lr_p_value_ao_trend1,sig_or_not_ao_trend1 ,the_ao_effect, trend_effect, file = "ao_trend_scale_parameter.RData")
